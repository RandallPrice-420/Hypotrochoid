...
public class KnobSciFiPlus : Control
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        Rectangle rect = new Rectangle(cx - size / 2 + 5,
                                       cy - size / 2 + 5,
                                       size - 10,
                                       size - 10);
        ...
        using (SolidBrush shadow = new SolidBrush(Color.FromArgb(60, glow)))
        {
        ...
        using (TextureBrush tb = new TextureBrush(CreateCarbonFiberTexture()))
        {
            g.FillEllipse(tb, rect);
        }

        // --- Metallic Rim ---
        using (Pen rim = new Pen(Color.FromArgb(120, 200, 255), 4))
        {
        ...
        using (Pen arc = new Pen(glow, _hover ? 8 : 6))
        {
        ...
        // --- Center Glow ---
        Rectangle inner = new Rectangle(rect.X + 20,
                                        rect.Y + 20,
                                        rect.Width - 40,
                                        rect.Height - 40);
        ...
        using (GraphicsPath gp = new GraphicsPath())
        {
            ...
            using (PathGradientBrush pgb = new PathGradientBrush(gp))
            {
                ...
                pgb.SurroundColors = new[] { Color.FromArgb(10, glow) };
    ...
    private Bitmap CreateCarbonFiberTexture()
    {
        Bitmap bmp = new Bitmap(8, 8);
        using (Graphics g = Graphics.FromImage(bmp))
        {
            g.Clear(Color.FromArgb(25, 25, 30));
            using (Pen p = new Pen(Color.FromArgb(40, 40, 50), 2))
            {
